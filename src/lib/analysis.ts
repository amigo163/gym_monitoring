...
/** Per-exercise, per-workout best performance, ordered by date. */
export function buildExerciseSessions(rows: SetRow[]): Map<string, ExerciseSession[]> {
  ...
  for (const [exercise, byWorkout] of grouped) {
    ...
    for (const [workoutId, sets] of byWorkout) {
      ...
      sessions.push({
        ...
        volume: sets.reduce((acc, s) => acc + s.volume, 0),
        sets: sets.length,
...
export function findPersonalRecords(
  ...
  const metric: Record<PrKind, (s: ExerciseSession) => number> = {
    e1rm: (s) => s.bestE1rm,
    weight: (s) => s.topWeight,
    volume: (s) => s.volume,
  }
  for (const [exercise, list] of sessions) {
    const muscle = muscleGroupFor(exercise)
    for (const kind of Object.keys(metric) as PrKind[]) {
      let best: number | null = null
      for (const s of list) {
        const v = metric[kind](s)
        if (v <= 0) continue
        if (best != null && v > best + 1e-6) {
          const set = bestSetByWorkout.get(`${exercise}|${s.workoutId}`)
          prs.push({
            exercise,
            muscle,
            date: s.date,
...
/**
 * Stretches of ≥ `window` sessions where the best e1RM (or top weight when e1RM
 * isn't estimable) never exceeded the value at the start of the stretch.
 */
export function detectPlateaus(list: ExerciseSession[], window = 4): Plateau[] {
  const value = (s: ExerciseSession) => s.bestE1rm || s.topWeight
...
export function mostImproved(sessions: Map<string, ExerciseSession[]>, minSessions = 3, top = 8) {
  const out: { exercise: string; muscle: MuscleGroup; changePct: number; from: number; to: number; sessions: number }[] = []
  for (const [exercise, list] of sessions) {
    if (list.length < minSessions) continue
    const val = (s: ExerciseSession) => s.bestE1rm || s.topWeight
    ...
    if (from <= 0) continue
    out.push({ exercise, muscle: muscleGroupFor(exercise), changePct: ((to - from) / from) * 100, from, to, sessions: list.length })
  }
